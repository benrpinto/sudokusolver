...
class SudokuPuzzle:
    ...
    def quickValidate(self, row:int, col:int, toCheck:int)->bool:
        ...
        if(row >= NUM_DIGITS or row < 0 or col >= NUM_DIGITS or col < 0):
            ...
            toReturn = toReturn and not(bool( digitFlag & self.rowDigits[row]))
            toReturn = toReturn and not(bool( digitFlag & self.colDigits[col]))
            toReturn = toReturn and not(bool( digitFlag & self.boxDigits[box]))

        return toReturn

    def placeDigit(self, row:int, col:int, toPlace:int):
        if(     row >= NUM_DIGITS or row < 0 or\
                col >= NUM_DIGITS or col < 0):
